...
def init_db():
    ...
    for tid, name in admins:
        try:
            db.execute("INSERT OR IGNORE INTO admins (telegram_id, name) VALUES (?,?)", (tid, name))
        except:
            pass
    db.commit()
    db.close()
...
def get_all_admins():
    db = get_db()
    r = db.execute("SELECT telegram_id FROM admins").fetchall()
    db.close()
